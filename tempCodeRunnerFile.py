...
# Run this first on the terminal: export GOOGLE_API_KEY="AIzaSyDpmKD1hLhYBdrLXXzUIiIGOkR9PWEaG2s"



# To see which metadata keys were actually considered in the final output
ALLOWED_FIELDS = {"main_components","length_m","num_piers","materials","missing_components","notes","bridge_type"}
...
def build_prompt(meta: Dict[str, Any], k:int=1) -> str:
    ...
    meta_llm = {fld: meta.get(fld) for fld in [
        ...
        "materials","missing_components","notes","target_tokens"
    ]}
    ...
    return (
        ...
        "Describe only the object itself — its geometry, structure, materials, and potential usage.\n\n"
        ...
        "- The Q&A pairs should focus on distinct and meaningful aspects (e.g., material, purpose, users).\n"
...
def validate_against_metadata(caption:str, meta:Dict[str,Any]) -> List[str]:
    ...
    if any(w in caption.lower() for w in ["color","paint","city","country","age","year built"]) and not any(k in meta for k in ["color","location","year_built"]):
        errs.append("possible speculation (color/location/age)")
    return errs
